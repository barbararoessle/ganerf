...
class Scannetpp(DataParser):
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        if "test_frames" in meta:
            all_frames = meta["frames"] + meta["test_frames"]
        else:
        ...
        elif "test_frames" in meta:
            num_train_images = len(meta["frames"])
            if split == "train":
                indices = np.arange(0, num_train_images)
            elif split in ["val", "test"]:
                indices = np.arange(num_train_images, num_train_images + len(meta["test_frames"]))
        else:
            # filter image_filenames and poses based on train/eval split percentage
